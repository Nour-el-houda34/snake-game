...
#include "utils.hpp"////ajouter pour resoudre probleme clamp n'est pas membre de std
...
void GamePlay::Init()
{
    m_context->m_assets->AddTexture(GRASS, "green.png", true);
    m_context->m_assets->AddTexture(FOOD, "food.png");
    m_context->m_assets->AddTexture(WALL, "white.png", true);
...
*/
               /* x = clamp(rand() % m_context->m_window->getSize().x, 16, m_context->m_window->getSize().x - 2 * 16);
                y = clamp(rand() % m_context->m_window->getSize().y, 16, m_context->m_window->getSize().y - 2 * 16);
*/
                x = clamp(static_cast<unsigned int>(rand() % m_context->m_window->getSize().x), 16u, static_cast<unsigned int>(m_context->m_window->getSize().x - 2 * 16));
               y = clamp(static_cast<unsigned int>(rand() % m_context->m_window->getSize().y), 16u, static_cast<unsigned int>(m_context->m_window->getSize().y - 2 * 16));
